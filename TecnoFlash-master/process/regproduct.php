<html>
    ...
    <body>
        ...
        if(!$codeProd=="" && !$nameProd=="" && !$cateProd=="" && !$priceProd=="" && !$modelProd=="" && !$marcaProd=="" && !$_FILES['img']['name']==""){
            ...
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$_FILES['img']['name'])){
                    if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, Precio, Modelo, Marca, Imagen, Nombre", "'$codeProd','$nameProd','$cateProd','$priceProd', '$modelProd','$marcaProd','".$_FILES['img']['name']."','$adminProd'")){
                       echo '
